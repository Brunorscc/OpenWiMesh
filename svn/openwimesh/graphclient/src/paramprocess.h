...
typedef struct argvParamItemSt {
    char key[3];        // In the format "-f\0"
    char *extKey;       // In the format "--format\0"
    bool boolVal;       // If true, no value will be expected
    bool req;           // If this is a required parameter
    char *value;        // Value for the attribute, memory must be dynamically assigned to it
    char *helpMsg;      // Help message for this parameter
    struct argvParamItemSt *next;
} argvParamItem;

typedef struct argvParamListSt {
    argvParamItem *start;
    argvParamItem *end;
} argvParamList;
